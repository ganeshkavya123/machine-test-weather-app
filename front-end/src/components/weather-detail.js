...
const WeatherDetail = () => {
    ...
    const fetchWeatherDetails = async () => {
      try {
        const response = await axios.get(
          `http://localhost:3001/api/location-weather?City=${city}`
        );
  ...
  const getWeatherIcon = (weatherName, sizeClass) => {
    switch (weatherName) {
      case "Clear":
        return (
          <WiDaySunny size={118} className={`text-yellow-500 ${sizeClass}`} />
        );
      case "Clouds":
      ...
      case "Thunderstorm":
        return (
          <WiThunderstorm
            size={118}
            className={`text-yellow-500 ${sizeClass}`}
          />
        );
      case "Fog":
      case "Mist":
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          ...
          <div href="#" class="solid-whether-card mt-12">
            {weatherDetails ? (
              <div>
                ...
                <p className="weather-card-today flex items-center justify-center">
                  Today
                ...
                <div className="flex items-center mr-5 mt-5 mb-5">
                  {getWeatherIcon(
                    weatherDetails.WeatherName,
                    "text-3xl md:text-5xl"
                  )}
                  <span className="font-bold-temp text-3xl md:text-5xl font-medium ml-2">
            ...
            <div className="row-span-3 grid-item p-4">
              <ul className="flex flex-wrap items-center justify-center text-gray-900 dark:text-white">
                {times.slice(0, 5).map((item, index) => (
                  <li
                    key={index}
                    className="me-4 md:me-6 flex flex-col items-center p-4"
                  >
                    <span>{item.time}</span>
        ...
        <div className="col-span-2 bg-none p-12 shadow rounded-lg">
          ...
          <p className="font-normal text-gray-700 dark:text-gray-400">
            ...
            malesuada lacus eget accumsan auctor.
          </p>
        </div>
      </div>
    </div>
